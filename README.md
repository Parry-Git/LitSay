 ...
 * **前后端分离**: 采用经典的前后端分离架构，前端 Vue.js 负责视图渲染和用户交互，后端 Flask 提供 RESTful API 进行数据处理和业务逻辑，实现了高效的并行开发协作和灵活的部署。
 * **数据库选择**: 选用 **OceanBase** 数据库，因为它具备高可移植、可扩展的特性，能够支持未来更大规模的文献数据管理需求，同时也满足了课程设计对于新型数据库技术的探索要求。
 * **AI 能力集成**: 通过集成 Gemini API，实现了文献元数据的**智能解析**，将传统的手动录入变为自动化提取，极大地提升了用户导入文献的效率和体验。
